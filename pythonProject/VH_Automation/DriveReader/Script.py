...
def readFiles(parent):
    ...
    for f in file_list:
        if f['mimeType']=='application/vnd.google-apps.folder': # if folder
            ...
            Files.append({ "id":f["id"], "title":f["title"],"email":f["owners"][0]["emailAddress"], "status: ": 0, "CreatedDate":f["createdDate"] })            
...
for files in AllFiles:
    ...
    if status:
        summary.append({'Title':fileName, 'Extension': extension, 'Email' : files['email'],'Client':client_name[0],'Domain':client_name[1] ,'File Size': size ,'Uploaded (by Company)':files['CreatedDate'] ,'status': status,   })
    # trash(files['id'])
    else:
        continue
